...
static bool dataDecl(SymbolTable * symTab, VarDeclNode * decl, TypeNode * typeNode){
	...
	if (varObj->getBaseType() == BaseType::VOID){
		...
		Symbol * sym = new Symbol(VAR, ErrorType::produce(), varName);
	...
	Symbol * sym = new Symbol(VAR, dataType, varName);
	decl->ID()->attachSymbol(sym);
	symTab->insert(sym);
	return true;
}
...
bool FnDeclNode::nameAnalysis(SymbolTable * symTab){
	...
	if (validName && validFormals){
		...
		Symbol * fnSym = new Symbol(FN, fnType, fnName);
...
void IDNode::attachSymbol(Symbol * symbolIn){
